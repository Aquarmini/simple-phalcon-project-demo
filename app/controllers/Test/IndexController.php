...
class IndexController extends Controller
{
    ...
    public function requestAction()
    {
        ...
        dump('获取所有输入参数', $request->get());                          //默认获取所有的请求参数返回的是array效果和获取$_REQUEST相同
        dump('获取输入参数 wen', $request->get('wen'));                     //获取摸个特定请求参数key的valuer和$_REQUEST['key']相同
        dump('获取输入参数 url 默认值', $request->getQuery('url', null, 'url'));   //获取get请求参数,第二个参数为过滤类型,第三个参数为默认值
        dump('访问方法', $request->getMethod());                    //获取请求的类型如果是post请求会返回"POST"
        dump('是否是Ajax请求', $request->isAjax());                       //判断请求是否为Ajax请求
        dump('是否是POST请求', $request->isPost());                       //判断是否是Post请求类似的有(isGet,isPut,isPatch,isHead,isDelete,isOptions等)
        dump('获取所有输入的头文件', $request->getHeaders());                   //获取所有的Header,返回结果为数组
        dump('获取输入的头 Content-Type', $request->getHeader('Content-Type'));      //获取Header中的的莫一个指定key的指
        dump('获取请求的URI', $request->getURI());                       //获取请求的URL比如phalcon.w-blog.cn/phalcon/Request获取的/phalcon/Request
        dump('获取请求的域名', $request->getHttpHost());                  //获取请求服务器的host比如phalcon.w-blog.cn/phalcon/Request获取的phalcon.w-blog.cn
        dump('获取服务器IP地址', $request->getServerAddress());             //获取当前服务器的IP地址
        dump('获取Raw请求json字符', $request->getRawBody());                   //获取Raw请求json字符
        dump('获取Raw请求json 数组', $request->getJsonRawBody());               //获取Raw请求json字符并且转换成数组对象
        dump('获取请求是http还是https', $request->getScheme());                    //获取请求是http请求还是https请求
        dump('获取服务器参数', $request->getServer('REMOTE_ADDR'));       //等同于$_SERVER['REMOTE_ADDR']
    }
